...
class App:
    ...
    def load_packages(truck):
        for index in truck.truck_packages:
            ...
            if address_id not in truck.truck_destinations:
    ...
    @staticmethod
    def set_initial_next_destination(truck):
        if len(truck.truck_priority_destinations) > 0:
            truck.truck_next = Distance().next_closest_stop(truck.truck_pos, truck.truck_priority_destinations)
        else:
            truck.truck_next = Distance().next_closest_stop(truck.truck_pos, truck.truck_destinations)
        truck.truck_goal = float(truck.truck_next[1])
    ...
    def set_truck_destinations(self, truck):
        if truck.truck_curr_pos >= truck.truck_goal:
            ...
            truck.truck_pos = truck.truck_next[0]
            self.unload_packages(truck.truck_packages, truck.truck_priority, truck.truck_pos)
            truck.truck_destinations.remove(truck.truck_next[0])
            if len(truck.truck_priority_destinations) > 0:
                truck.truck_priority_destinations.remove(truck.truck_next[0])
                truck.truck_next = Distance().next_closest_stop(truck.truck_pos, truck.truck_priority_destinations)
            elif len(truck.truck_destinations) > 0:
                truck.truck_next = Distance().next_closest_stop(truck.truck_pos, truck.truck_destinations)
            elif len(truck.truck_destinations) < 1 and truck.truck_next[0] != 0:
                truck.truck_destinations.append(0)
                truck.truck_next = (0, Distance().distance_home(truck.truck_pos))
            truck.truck_goal = float(truck.truck_next[1])
    ...
    def run_simulation(self, time):
        ...
        while not ((len(self.truck1.truck_destinations) < 1 and
                len(self.truck2.truck_destinations) < 1 and
                len(self.truck3.truck_destinations) < 1) or
               (self.current_time.time() >= self.simulation_end)):
